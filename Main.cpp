...
CREATED A ROOM WITH A WALL FAN AND A SELF IN IT.

On key press of ‘s’, the fan starts spinning the blades.
On key press of ‘t’, the fan stops.
On pressing ‘u’ (up), ‘d’(down), ‘l’(left) and ‘r’(right), the fan starts moving up, down, left and
right respectively on the current wall its located in.
